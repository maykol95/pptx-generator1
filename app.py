...
def calcular_dimensiones(img_path, fotos_por_slide, encabezados_count):
    try:
        with Image.open(img_path) as img:
            ...
            max_width = 11 / fotos_por_slide - 0.5  # ajustado
            encabezado_space = 0.3 + 0.25 * min(encabezados_count, 6)
            max_height = 6 - encabezado_space  # ajustado
...
# Configuración de estilo de encabezados
...
if excel_file:
    ...
    st.subheader("Configuración de presentación")
    ...
    if st.checkbox("¿Deseas aplicar filtros?"):
            ...
            if not imagen_col:
                ...
                for nombre, path in archivos_generados:
                    with open(path, "rb") as f:
                        st.download_button(
                            label=f"\U0001F4E5 Descargar {nombre}.pptx",
                            data=f,
